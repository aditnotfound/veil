...
## 13. Implementation checkpoint (23 September 2026)
...
| Credential storage | The installed `tauri-plugin-keychain` desktop backend exposes no working commands. It was replaced with native OS credential storage through `keyring`. Selected AI/STT provider variables are migrated only after the native save verifies a readback; plaintext localStorage is then reduced to a provider pointer. New selections never write variables to localStorage. A Windows Credential Manager roundtrip test passed and deleted its temporary entry. | Packaged upgrade using real existing settings, macOS/Linux stores, custom CURL strings that may contain literal secrets, and full storage/log/export inspection. SEC-01 is not yet complete. |
| Verification | Forty-four Node tests, eighteen Python SQLite/report/replay tests, and seven Rust tests pass locally, including native credential, knowledge rollback, answer/deep/ledger/planner/revision/invalidation deletion, retention parsing/cutoffs and dependent-row pruning, correction restore audit chaining, migration-15-to-16 invalidation preservation, tier-specific regeneration, concurrent-correction rejection, bounded source-linked regeneration history, exact saved-session joins, transcript correction auditing and stale-answer joins, candidate-ledger classification and prompt bounds, planner snapshot/schema/stale-revision behavior, deep-model override isolation, checkpoint reconstruction, deep-prompt labeling, useful-latency validation, and independent hard-answer rubric validation. TypeScript, Rust tests, production frontend build, and the native debug build pass after a clean rebuild. The explicit smoke-identifier Windows debug executable remained alive through its isolated smoke interval; its database reported migration 15, the revision and invalidation tables, the correction audit/invalidation/cleanup triggers, and SQLite integrity `ok`. The isolated NSIS package built, installed, upgraded a seeded migration-14 database, relaunched after correction, and uninstalled with exit code 0. Its upgraded database retained seeded records and passed integrity checks. The NSIS uninstaller removed the application directory but retained the isolated AppData database; the Meetings page now discloses that behavior and provides the local clear-all control. A Windows GitHub Actions workflow repeats Node, Python, frontend, and Rust checks on pushes and pull requests; remote run `35891129541` passed all four gates for audited correction restore commit `647c5d4`. Live-caption tests use a mock WebSocket; JEV tests use a fake HTTP transport, including malformed choices, HTTP failure, timeout, and cancellation that ignores abort. | Real-provider UI invocation and regeneration, actual microphone/loopback, Deepgram and answer-provider credentials, packaged migration 16, a labeled useful-card set, independently graded hard-question set, provider-backed JEV and planner evaluation, macOS/Linux gates. |
